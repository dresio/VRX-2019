...
void totems::totemStuffCallback(visualization_msgs::MarkerArray msg)
{
    ...
    for (int i = 0; i < msg.markers.size(); i++) {
        if (msg.markers[i].ns == "totem") {
            ...
            if (msg.markers[i].color.r == 1) {
                // ROS_INFO("The totem is RED");
                curr_color = 0;
            }
            else if (msg.markers[i].color.g == 1) {
                curr_color = 1;
            }
            else if (msg.markers[i].color.b == 1) {
                curr_color = 2;
            }
            else {
                curr_color = -1;
...
void totems::loop()
{
    switch (this->task)
    {
        ...
        case SEARCH_TOTEMS_1:
        {
                        ...
                        if (totems_arr[i].color_detected) {
                            if (totems_arr[i].color == j && totems_order[j] == k) {
                                if (k == 0) {
                                    ...
                                    ROS_INFO("LINE %d", 413);
                                    ROS_INFO("Position of totem = [%g, %g]", totems_arr[i].totem_pos.x, totems_arr[i].totem_pos.y);
                                    this->task = GOTO_TOTEM_1;
                                    break;
                                }
                                else {
            ...
            if (totem_found[0] == true) {
                this->task = GOTO_TOTEM_1;
                break;
            }
            else if (totem_found[1] && totem_found[2]) {
                ...
                if (client_goal.call(goto_srv)) {
                    ROS_INFO("Service call to send-goal server was successful from SEARCH_TOTEMS_1 : 496");
                    ROS_INFO("goal_heading_enu = %g deg", goto_srv.response.goal_heading_enu);
                }
                else {
                    ROS_INFO("Service call failed");
                ...
                if (client_goal.call(goto_srv)) {
                    ROS_INFO("Service call to send-goal server was successful from SEARCH_TOTEMS_1 : 586");
                    ROS_INFO("goal_heading_enu = %g deg", goto_srv.response.goal_heading_enu);
                }
                else {
                    ROS_INFO("Service call failed");
        ...
        case GOTO_TOTEM_1:
        {
            ...
            // The position provided here, is with respect to the outer circle around
            // the totem, which defines the region prior to the actual trajectory-circle.
            // The outer circle is the area where the circling operation begins.
            // These coordinates are in ENU convention, in agreement with the move_base node
            geometry_msgs::Pose totem_goal;
            // This is the actual position of the centroid of the totem
            totem_goal.position.x = 0;
            totem_goal.position.y = 30;
        ...
        case CIRCLE_TOTEM_1:
        {
            ...
            if (client_circle.call(circle_srv)) {
                ROS_INFO("Service call to circle-target server was successful from CIRCLE_TOTEM_1 : 686");
            }
            else {
                ROS_INFO("Service call failed");
                finished = true;
            ...
            for (unsigned long i = 0; i < circle_srv.response.circle_path.poses.size(); i++) {
                ...
                if (client_goal.call(goto_srv)) {
                    ROS_INFO("Service call to send-goal server was successful from CIRCLE_TOTEM_1 : 718");
                    ROS_INFO("goal_heading_enu = %g deg", goto_srv.response.goal_heading_enu);
        ...
        case GOTO_TOTEM_2:
        {
            ...
            if (client_goal.call(goto_srv)) {
                ROS_INFO("Service call to send-goal server was successful from GOTO_TOTEM_2");
                ROS_INFO("goal_heading_enu = %g deg", goto_srv.response.goal_heading_enu);
            }
            else {
                ROS_INFO("Service call failed");
        ...
        case CIRCLE_TOTEM_2:
        {
            ...
            if (client_circle.call(circle_srv)) {
                ROS_INFO("Service call to circle-target server was successful from CIRCLE_TOTEM_2");
            }
            else {
            ...
            for (unsigned long i = 0; i < circle_srv.response.circle_path.poses.size(); i++) {
                ...
                if (client_goal.call(goto_srv)) {
                    ROS_INFO("Service call to send-goal server was successful from CIRCLE_TOTEM_2");
                    ROS_INFO("goal_heading_enu = %g deg", goto_srv.response.goal_heading_enu);
                    printf("circle-goal from server = [\n");
                    printf("goal_x = %g\n", circle_srv.response.circle_path.poses[i].position.x);
                    printf("goal_y = %g\n", circle_srv.response.circle_path.poses[i].position.y);
        ...
        case FINISH:
        {
            ...
            if (client_goal.call(goto_srv)) {
                ROS_INFO("Service call to send-goal server was successful from FINISH");
                ROS_INFO("goal_heading_enu = %g deg", goto_srv.response.goal_heading_enu);
            }
            else {
                ROS_INFO("Service call failed");
